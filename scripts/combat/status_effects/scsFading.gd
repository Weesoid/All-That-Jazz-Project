static func applyEffects(target: ResCombatant, status_effect: ResStatusEffect):
	...
	if CombatGlobals.randomRoll(1.0 + target.BASE_STAT_VALUES['grit']) and status_effect.duration != status_effect.MAX_DURATION - 1 and status_effect.duration > 0 and !CombatGlobals.getCombatScene().has_node('QTE'):
