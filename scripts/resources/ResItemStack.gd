...
func add(count: int, show_prompt=true):
	if (count + STACK <= MAX_STACK and MAX_STACK != 0) or MAX_STACK == 0:
		STACK += count
		if show_prompt: OverworldGlobals.showPlayerPrompt('Added [color=yellow]%s (%s)[/color].' % [NAME, STACK])
	else:
		STACK = MAX_STACK
		if show_prompt: OverworldGlobals.showPlayerPrompt('[color=yellow]%s[color=white] max stack reached.' % [NAME])
