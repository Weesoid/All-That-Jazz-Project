...
signal level_up
signal experience_added(level_upped:bool)
...
func getRequiredExp(level_offset:int=0) -> int:
	var baseExp = 500.0
	var expMultiplier = 1.25
	var gain = pow(expMultiplier ** ((team_level+level_offset) - 1), 1.0/3.0) * baseExp # Chng to cubrrt
	return int(gain)
