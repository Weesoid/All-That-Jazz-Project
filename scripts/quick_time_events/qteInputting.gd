...
func displayInputs():
	...
	for i in range(3): # Change to MOVE ui input
		...
		match inputs[i]:
			'ui_left': icon.texture = load("res://images/sprites/arrow_left.png")
			'ui_right': icon.texture = load("res://images/sprites/arrow_right.png")
			'ui_down': icon.texture = load("res://images/sprites/arrow_down.png")
			'ui_up': icon.texture = load("res://images/sprites/arrow_up.png")
		if i != 0:
